...
public class SeckillBizServiceImpl implements SeckillBizService {
    ...
    public void seckillOrderSubmit(Long skuId, String skuIdStr) {
        ...
        Long userId = AuthContextHolder.getUserAuth().getUserId();
        String seckillCode = MD5.encrypt(DateUtil.formatDate(new Date()) + userId + "" + skuId);
        //前端带来的码符合规则，而且redis也有
        if (!seckillCode.equals(skuIdStr) || !redisTemplate.hasKey(RedisConst.SECKILL_CODE_PREFIX + skuIdStr)) {
    ...
    public OrderInfo getSeckillOrder(String code, Long skuId) {
        Long userId = AuthContextHolder.getUserAuth().getUserId();
        String encrypt = MD5.encrypt(DateUtil.formatDate(new Date()) + userId + "" + skuId);
    ...
    public Long saveSeckillOrder(OrderInfo orderInfo) {
        ...
        String code = MD5.encrypt(DateUtil.formatDate(new Date()) + userId + "" + skuId);
        String json = redisTemplate.opsForValue().get(RedisConst.SECKILL_ORDER_PREFIX + code);
        OrderInfo redisObj = JSONs.toObj(json, OrderInfo.class);
        ////从redis拿到这个单，把前端提交的收货地址补充以下
        ...
        redisTemplate.opsForValue().set(RedisConst.SECKILL_ORDER_PREFIX + code,JSONs.toStr(redisObj));
