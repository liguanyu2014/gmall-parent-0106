...
public class GlobalAuthFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        /**
         * .mutate() : "变异" 把老请求变成一个新请求,老请求有的都有,且返回的是一个ServerWebExchange.Builder
         *              能自己添加请求头
         */
    ...
    private Mono<Void> locationToPage(ServerHttpResponse response, String loginPage) {
        ...
        //ResponseCookie.from : 获取服务器定义的cookie的生成器的工厂方法，该cookie以名称-值 对开始，还可能包括属性
        ResponseCookie cookie = ResponseCookie.from("token", "xxxx")
                .maxAge(0)
                .domain(".gmall.com")
                .path("/")
                .build();
        //清除留存的用户信息(登录用户:admin)
